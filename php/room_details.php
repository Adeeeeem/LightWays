<?php
	...
	if ( isset($room) && !empty($room) )
	{
		if (is_numeric($room))
		{
			...
			/* Preparing Request */
			$request = "SELECT FLOOR_ID AS floor, ROOMS.ROOM_ID AS room, ROOM_NAME AS name, ROOM_WIDTH AS width, ROOM_HEIGHT AS height, DEVICE_ID AS device, DEVICE_PIN as pin, DEVICE_TYPE AS type, CARD_ID AS card, DEVICE_LINE AS lin, DEVICE_COLUMN AS col, (SELECT GROUP_COLOR FROM GROUPS NATURAL JOIN DEVICES WHERE DEVICE_ID = device) AS color FROM ROOMS LEFT JOIN DEVICES ON DEVICES.ROOM_ID = ROOMS.ROOM_ID WHERE ROOMS.ROOM_ID = :room;";
			/* Preparing Statement */
