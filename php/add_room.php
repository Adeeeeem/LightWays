		...
		if ( (is_string($name)) && (is_numeric($width)) && (is_numeric($height)) && (is_numeric($floor)) )
		{
			...
			if ($statement->rowCount()) // If Room Added Successfully, Add Devices to it
			{
				...
				if ($_SESSION["6C3Zq5Bpwm"] != "lightways")
				{
					/* Add to History */
					/* Preparing Request */
					$request = "INSERT INTO HISTORY (HISTORY_USER, HISTORY_TYPE, HISTORY_DATA_ID, HISTORY_DATA, HISTORY_DATE, HISTORY_TIME, HISTORY_OPTION) VALUES (:user, 'ADD', :room, 'ROOM', CURRENT_DATE, CURRENT_TIME, :name);";
					/* Preparing Statement */
					$statement = $DB_CONNECTION->prepare($request);
					/* Binding Parameter */
