...
-- Events
--
CREATE DEFINER=`root`@`localhost` EVENT `SYSTEM_STATISTICS_CONFIGURATION` ON SCHEDULE EVERY 1 DAY STARTS '2020-05-05 23:55:00' ON COMPLETION PRESERVE ENABLE COMMENT 'SYSTEM_STATISTICS_CONFIGURATION' DO INSERT INTO HISTORY (HISTORY_USER, HISTORY_TYPE, HISTORY_DATA_ID, HISTORY_DATA, HISTORY_DATE, HISTORY_TIME)
SELECT 'SYSTEM', 'RESET', DEVICE_ID, 'DEVICE', CURRENT_DATE, CURRENT_TIME FROM DEVICES WHERE DEVICE_STATUS = 'ON'$$
