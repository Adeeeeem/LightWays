...
CREATE TABLE `SCENES` (
  ...
  `SCENE_START` datetime NOT NULL,
  `SCENE_END` datetime NOT NULL,
...
--
-- Indexes for table `SCENES_DEVICES`
--
ALTER TABLE `SCENES_DEVICES`
  ADD PRIMARY KEY (`SCENE_ID`,`DEVICE_ID`),
  ADD KEY `fk_device_SCENES` (`DEVICE_ID`);
...
--
ALTER TABLE `FLOORS`
  MODIFY `FLOOR_ID` int(10) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=4;
...
--
-- Constraints for table `SCENES_DEVICES`
--
ALTER TABLE `SCENES_DEVICES`
  ADD CONSTRAINT `fk_device_SCENES` FOREIGN KEY (`DEVICE_ID`) REFERENCES `DEVICES` (`DEVICE_ID`) ON DELETE CASCADE ON UPDATE CASCADE,
  ADD CONSTRAINT `fk_scene_SCENES` FOREIGN KEY (`SCENE_ID`) REFERENCES `SCENES` (`SCENE_ID`) ON DELETE CASCADE ON UPDATE CASCADE;
COMMIT;
